...
Lambda.prototype._replaceNative = function (program, codeDirectory, callback) {
  var _this = this;
  var nativesDir = codeDirectory + '/native_packages';
  var modulesDir = codeDirectory + '/node_modules';

  async.waterfall([
    function (taskCallback) {
      fs.readdir(nativesDir, taskCallback);
    },
    function (contents, taskCallback) {
      async.filter(
          contents,
          function (element, filterCallback) {
            return fs.stat(path.join(nativesDir, element), function (err, stats) {
              filterCallback(stats.isDirectory())
            });
          },
          function (dirs) {
            return taskCallback(null, dirs);
          }
      );
    },
    function (dirs, taskCallback) {
      var dirIndex = 0;
      async.whilst(
          function () {
            return dirIndex < dirs.length;
          },
          function (whilstCallback) {
            rimraf(path.join(modulesDir, dirs[dirIndex]), whilstCallback);
            dirIndex++;
          },
          taskCallback
      );
    ...
    function (taskCallback) {
      ncp(codeDirectory + '/native_packages', codeDirectory + '/node_modules', taskCallback);
    }
  ], function (err) {
    if (err) {
      console.error(err);
...
Lambda.prototype._npmInstall = function (program, codeDirectory, callback) {
  var _this = this;
  exec('npm install --production --msvs_version=2015 --prefix ' + codeDirectory, function (err) {
    if (err) {
      throw err;
    }

    if (program.winPlatform) {
      _this._replaceNative(program, codeDirectory, callback)
    } else {
      return callback(null, true);
    }
  });
